...
CONFIG = {
    ...
    "CRYPTO_SYMBOLS": [
        "BTCUSDT", "ETHUSDT", "LTCUSDT", "DOGEUSDT",
    ],   # Binance format -- Quotex live crypto list se match
    "FOREX_PAIRS": [
    ...
    "CRYPTO_INTERVALS": ["1m", "3m"],     # Binance format
...
def get_crypto_candles(symbol, interval, limit=100):
    """Binance public API se OHLC candle data leta hai (koi key nahi chahiye)."""
...
def score_timeframe(df, cfg):
    ...
    min_len = max(cfg["EMA_SLOW"], cfg["BB_PERIOD"], cfg["MACD_SLOW"] + cfg["MACD_SIGNAL"]) + 2
...
def check_market_once(cfg):
    ...
    # ---- Crypto: har cycle mein SAB check hote hain (Binance -> koi rate limit nahi) ----
    for symbol in cfg["CRYPTO_SYMBOLS"]:
        try:
            candles_by_tf = {
                tf: get_crypto_candles(symbol, tf) for tf in cfg["CRYPTO_INTERVALS"]
            }
...
def handle_signal(cfg, state, key, direction, results):
    ...
    for tf, (tf_dir, bulls, bears, price, details) in results.items():
        ...
        tf_lines.append(
            f"  {tf}: {tf_dir} (bullish {bulls}/5, bearish {bears}/5) "
...
def find_best_instant_signal(cfg):
    ...
    for symbol in cfg["CRYPTO_SYMBOLS"]:
        try:
            candles_by_tf = {tf: get_crypto_candles(symbol, tf) for tf in cfg["CRYPTO_INTERVALS"]}
            direction, results = generate_multi_timeframe_signal(candles_by_tf, cfg)
...
def handle_predict_request(cfg, chat_id, args):
    ...
    try:
        if market == "crypto":
            df = get_crypto_candles(actual_symbol, timeframe)
        else:
    ...
    message = (
        ...
        f"Call: *{call}*\n"
        f"Bullish votes: {bullish}/5 | Bearish votes: {bearish}/5\n"
